#!/usr/bin/env python3
"""
Deep Voice TTS Pipeline v4 - Male voices with accent options
Enhanced acronym detection and voice variety
Outputs .mp3 files to a single 'processed' directory
"""
...
class DeepVoiceTTS:
    ...
    def process_text_file(self, input_file, output_name=None, output_dir="processed"):
        """
        Process text file with deep voice generation.
        All output goes into a single directory (default: processed/).
        """
        ...
        # Generate audio chunks in a temp directory
        print(f"\n🎵 Generating deep voice audio...")
        audio_files = []

        with tempfile.TemporaryDirectory() as tmp_dir:
            for i, chunk in enumerate(tqdm(chunks, desc="Processing")):
                chunk_file = Path(tmp_dir) / f"chunk_{i:04d}.{self.output_format}"
            ...
            # Combine audio
            if audio_files:
                final_output = output_dir / f"{base_name}.{self.output_format}"
                print(f"\n🔄 Combining audio...")
        ...
        # Save metadata alongside the audio
        metadata_path = output_dir / f"{base_name}_metadata.json"
        with open(metadata_path, 'w') as f:
            json.dump(metadata, f, indent=2)

        print(f"\n✅ Complete! Output: {final_output}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Deep Voice TTS v4 with accent options")
    ...
    parser.add_argument("--format", choices=["mp3", "wav"], default="mp3",
                       help="Output format")
    parser.add_argument("--output-name", help="Custom output base name (without extension)")
    parser.add_argument("--output-dir", default="processed",
                       help="Output directory for all generated files (default: processed)")
    ...
    if args.list_voices:
        ...
        for voice in favorites:
            print(f"  {voice}")
        print("\nUsage examples:")
        print("  python deep_voice_tts_v4.py input.txt --voice p230")
        print("  python deep_voice_tts_v4.py input.txt --voice random")
        print("  python deep_voice_tts_v4.py input.txt --output-dir my_audio")
        return
